...
{
    public class GerenciarLotesArquivos : IGerenciarLotesArquivos
    {
        ...
        public GerenciarLotesArquivos(ILoteService loteService,
        ...
        {
            await ProcessarLoteArquivos();
        }


        public async Task ProcessarLoteArquivos()
        {
            ...
            foreach (var arquivo in arquivos)
            {
                ...
                try
                {
                    ...
                    if (arquivoProcessado is not null)
                        arquivosProcessados.Add(arquivoProcessado);
                }
                catch (Exception ex)
                {
                    ...
                    {
                        Sistema = "Processador-Lotes-Arquivos",
            ...
            //gravar lote com arquivos processados
            var lote = await _loteService.GerarLote(arquivosProcessados);

            await PublicarLoteArquivos(lote);
        }
        ...
        public async Task PublicarLoteArquivos(LoteViewModel lote)
        {
            ...
            var bytesMessage = Encoding.UTF8.GetBytes(JsonSerializer.Serialize(new EnvioFila { Lote = true, Id = lote.Id }));
